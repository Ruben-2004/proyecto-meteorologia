...
from src.weather.land_use import classify_land, create_buffers, intersect_land, land_use_percentage
...
class TestIntersectLand:
    ...
    def test_happy_path(self):
        buffers = gpd.GeoDataFrame(
            {"ciudad": ["A"], "geometry": [Point(0, 0).buffer(1)]},
            crs="EPSG:4326"
        )

        clc = gpd.GeoDataFrame(
            {"Code_18": [111], "geometry": [Point(0, 0).buffer(1)]},
            crs="EPSG:4326"
        )
    ...
    def test_edge_no_overlap(self):
        ...
        clc = gpd.GeoDataFrame(
            {"Code_18": [111], "geometry": [Point(100, 100).buffer(1)]},
            crs="EPSG:4326"
        )
...
class TestLandUsePercentage:
    ...
    def test_edge_empty(self):
        gdf = gpd.GeoDataFrame(
            {"ciudad": [], "categoria": [], 'area': [], "geometry": []},
            crs="EPSG:4326"
        )
